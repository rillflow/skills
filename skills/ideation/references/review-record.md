...
追加原理や発展法則など、一つの方法に複数の項目がある場合は、`evidence`で各項目の判断を示す記録を参照します。原典を読んだことと、現在の問題へ適用して得た結果を分けます。
...
`function-oriented-search`と`analogue-problems`を`used`にする場合、その道具の行に空でない`searches`配列が必要です。各検索には次を記録します。

| 項目 | 内容 |
|---|---|
| provider・query・source_url | 実際の検索先、検索語、検索画面または読んだ資料のURL |
| domain | 調べた分野。方法論の解説を読んだだけでは別分野の検索としない |
| results | 見つかった先行手段の`name`と`url`の配列。0件なら空配列にする |
| adaptation | 発見した仕組みを現在の問題へ移す条件と限界。0件なら探索で分かった不足 |

機能の言い換えと、TRIZの方法をキャッシュ内で調べる作業は、この検索実績に含めません。適用外なら`searches`は不要ですが、具体的な不一致を`reason`に記します。記録の存在は検索や解法の正しさの証明ではありません。
